...
- 在新的 Python 3.12.14 虚拟环境中按 requirements.txt 安装依赖。
- 在 Linux 检查环境运行全部 10 个自动化测试。
- 实际运行帮助、跳过服务的资源巡检、两种进程排序、日志分析和备份。
- 检查从项目目录之外启动时的相对路径行为，以及常见错误的退出状态。
- 当前检查环境没有可用的 systemd 服务管理器，真实服务状态需在 Ubuntu 主机验收。
- 修订版尚未在 Windows 主机重新运行，应在替换文件后运行全部测试及常用命令。
